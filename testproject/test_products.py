from testproject import settings as project_settings
...
if __name__ == '__main__':
    ...
    if arg == 'create':
        ...
        logging.info(f'Test products were created successfully. Their primary keys: {", ".join([str(pk) for pk in primary_keys[:-1]]) + f" and {primary_keys[-1]}"}.')
    elif arg == 'delete':
        if os.path.exists(FILENAME):
            ...
            for pk in primary_keys:
                try:
                    ...
                    logging.warning(f'Product with PK {pk} was not deleted, because it not exists')
        else:
            logging.critical(f'Bytes file ({FILENAME}) with test products PK list was not found! Have you created test products?')
